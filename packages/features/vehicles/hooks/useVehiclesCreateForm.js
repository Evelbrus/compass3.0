...
export const useVehiclesForm = ({ mode, vehicleData }) => {
    ...
    const [previewImage, setPreviewImage] = useState(vehicleData?.photoPath
     ...
     */
    const loadDrivers = useCallback(async (reset = false) => {
        if (loading)
            return;
        ...
        setLoading(true);
        try {
            const response = await fetchDrivers('', String(currentPage), String(perPage), 'createdAt', 'asc');
    ...
    const onSelectDriver = (driver) => {
        const currentDrivers = watch('vehicleDrivers') || [];
    ...
    const onRemoveDriver = (uuid) => {
        const currentDrivers = watch('vehicleDrivers') || [];
        setValue('vehicleDrivers', currentDrivers.filter((d) => d.driver.uuid !== uuid));
    };
    ...
    const onSubmit = useCallback(async (data) => {
        try {
            ...
            let photoPath = vehicleData?.photoPath ?? null;
            ...
            if (!response.ok) {
                ...
                const errorMessage = errorData.error?.message || `Failed ${action} vehicle: ${response.status}`;
            ...
            if (photoImage instanceof File && photoPath) {
                ...
                if (!uploadResponse.ok) {
                    ...
                    const uploadErrorMessage = uploadError.error?.message || uploadError.message || 'Error uploading image';
