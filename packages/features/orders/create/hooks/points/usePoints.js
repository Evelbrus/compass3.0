...
export const usePoints = ({ setErrorMessage, per_page = 4 }) => {
    const [points, setPoints] = useState(null);
    const [total, setTotal] = useState(0);
    const [currentPage, setCurrentPage] = useState(1);
    const [isLoading, setIsLoading] = useState(false);
    const prevSearchRef = useRef(undefined);
    const fetchAllPoints = useCallback(async (searchQuery = '', page = 1) => {
        try {
            setIsLoading(true);
    ...
    const refetchPoints = useCallback((searchQuery = '') => {
        setCurrentPage(1);
        fetchAllPoints(searchQuery, 1);
    }, [fetchAllPoints]);
    const fetchPointsByUuidsCallback = useCallback(async (uuids) => {
        try {
            ...
            setErrorMessage(error, 'Ошибка при получении точек по UUID');
            return null;
        }
    }, [setErrorMessage]);
