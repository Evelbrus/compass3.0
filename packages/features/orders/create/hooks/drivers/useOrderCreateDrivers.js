...
import { $selectedServiceLevel, $selectedVehicleType, $areValuesFromProps, setSelectedVehicleType, setSelectedServiceLevel, } from '@shared/lib/effector/orders/stateStore';
export const useOrderCreateDrivers = ({ setValue, assignedDriverId, setErrorMessage, }) => {
    ...
    const { drivers: rawDrivers, assignedDriver, isDriversLoading, refetchDrivers, fetchAssignedDriverData, page, perPage, setPage, total, serverTime, } = useDrivers({
    ...
    const validateDriverCompatibility = useCallback((driver) => {
        if (!driver.vehicleDriver?.vehicle)
            return false;
        const { vehicle } = driver.vehicleDriver;
        //If areValuesFromProps = false, do NOT check compatibility
        if (!areValuesFromProps)
            return true;
        return (vehicle.vehicleType === selectedVehicleType &&
    ...
    useEffect(() => {
        if (assignedDriverId && assignedDriver) {
            if (validateDriverCompatibility(assignedDriver)) {
                setSelectedDriverInfo(assignedDriver);
    ...
    const handleSearchDriverChange = useCallback((value) => {
    ...
    const handleDriverSelect = useCallback((driver) => {
        ...
        if (vehicle.vehicleType !== selectedVehicleType ||
    ...
    const handleDriverClick = useCallback((driverId) => {
