...
const Notification = ({ userSession }) => {
    ...
    const { notifications, clearNotifications, markAsRead, activeNotification, openModal, closeModal, } = useNotifications({ userSession });
    ...
    return (_jsxs(_Fragment, { children: [activeNotification && (_jsxs(_Fragment, { children: [activeNotification.action === Action.noted && (_jsx(OrderInfoModal, { isOpen: true, onClose: closeModal, notification: activeNotification })), activeNotification.action === Action.inProgress && (_jsx(OrderProgressModal, { isOpen: true, onClose: closeModal, notification: activeNotification })), activeNotification.action === Action.warning &&
                        userSession?.role === UserRole.Driver && (_jsx(WarningModal, { isOpen: true, onClose: closeModal, notification: activeNotification })), activeNotification.action === Action.warning &&
                        (userSession?.role === UserRole.Operator || userSession?.role === UserRole.Admin) && (_jsx(WarningAdminModal, { isOpen: true, onClose: closeModal, notification: activeNotification })), activeNotification.action === Action.success && (_jsx(OrderInfoModal, { isOpen: true, onClose: closeModal, notification: activeNotification }))] })), _jsxs("div", { className: "relative", children: [_jsxs("button", { onClick: () => setIsOpen(!isOpen), className: "p-2 rounded-full bg-[#2A3037] hover:bg-gray-100 shadow-md transition-colors group", "aria-label": "\u0423\u0432\u0435\u0434\u043E\u043C\u043B\u0435\u043D\u0438\u044F", children: [_jsx(LazyImage, { src: "/icons/bell.svg", alt: "notification-icon", className: "w-[18px] h-[18px] duration-200 filter invert-0 group-hover:invert" }), unreadCount > 0 && (_jsx("span", { className: "absolute top-0 right-0 bg-red-500 text-white rounded-full w-4 h-4 text-xs flex items-center justify-center", children: unreadCount }))] }), userSession?.role === UserRole.Driver && (_jsxs("button", { onClick: () => setIsDriverOpen(!isDriverOpen), className: "ml-2 p-2 rounded-full bg-[#2A3037] hover:bg-gray-100 shadow-md transition-colors group", "aria-label": "\u0423\u0432\u0435\u0434\u043E\u043C\u043B\u0435\u043D\u0438\u044F \u0432\u043E\u0434\u0438\u0442\u0435\u043B\u044F", children: [_jsx(LazyImage, { src: "/icons/driver-bell.svg", alt: "driver-notification-icon", className: "w-[18px] h-[18px] duration-200 filter invert-0 group-hover:invert" }), driverUnreadCount > 0 && (_jsx("span", { className: "absolute top-0 right-0 bg-blue-500 text-white rounded-full w-4 h-4 text-xs flex items-center justify-center", children: driverUnreadCount }))] })), isOpen && (_jsx("div", { ref: notificationRef, children: _jsx(NotificationList, { notifications: notifications, onClose: () => setIsOpen(false), onClear: clearNotifications, markAsRead: markAsRead }) })), isDriverOpen && (_jsx("div", { ref: notificationRef, children: _jsx(DriverNotificationList, { notifications: driverNotifications, onClose: () => setIsDriverOpen(false), openModal: openModal }) }))] })] }));
};
