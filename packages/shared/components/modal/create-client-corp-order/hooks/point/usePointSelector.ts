...
const usePointSelector = ({
                            initialPoints = [],
                            allPoints = [], // По умолчанию пустой массив
  ...
  // Обновляем точки и фильтрацию на основе allPoints и search
  useEffect(() => {
    ...
    setPoints(allPoints); // Все точки из пропса
    setFilteredPoints(filtered); // Отфильтрованные точки
  ...
  return {
    ...
    search,
    filteredPoints,
    loading: false, // Загрузка больше не нужна
    onOpenSelect,
    onSearchValueChange,
    handleSearchChange,
    onSelectPoint,
