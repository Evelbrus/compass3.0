...
export const LazyImage: React.FC<LazyImageProps> = ({
  ...
  overlay = false,
  overlayClassName = 'rounded-xl absolute top-0 left-0 w-full h-full bg-[#ffffff54] transition-colors duration-300 z-[1] group-hover:bg-transparent',
  position = 'relative',
}) => {
  ...
  if (!width || !height) {
    console.error(
      `LazyImage Error: Не удалось определить width/height для изображения "${src}". Убедитесь, что классы w-[*px] и h-[*px] переданы корректно.`,
    );
  }

  return (
    <div ref={imageRef} className={`${position} select-none overflow-hidden`}>
      {isVisible ? (
        <>
          <Image
            src={src}
