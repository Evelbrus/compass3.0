...
import { useOrderEditLogic } from './OrderEdit.logic';

interface OrderEditViewProps {
  orderId: string;
}

const OrderEditView: React.FC<OrderEditViewProps> = ({ orderId }) => {
  const {
    clients,
    points,
    drivers,
    ...
    handleSubmit,
    getAvailablePoints,
  } = useOrderEditLogic({ orderId });

  if (loading) {
    return <div>Loading order details for edit...</div>;
  }
