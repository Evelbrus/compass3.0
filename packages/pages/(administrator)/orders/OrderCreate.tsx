...
const OrderCreate = () => {
  ...
  const calculateBasePrice = (tariff: ExtendedTariff): number => {
    let total = tariff.price;
    tariff.tariffAdditionalServices.forEach((s) => {
      total += s.price;
    });
    return total;
  };

  const updateTariffs = async (serviceLevel?: string, vehicleType?: string) => {
    try {
      const tariffsData = await fetchTariffs(serviceLevel, vehicleType);
      setTariffs(tariffsData);
    } catch (error) {
      console.error(error);
      setMessage('Error fetching tariffs');
    }
  };
  ...
  const handleArrivalPointChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const selected = points.find((p) => p.uuid === e.target.value);
    if (selected) {
      setFormData((prev) => ({ ...prev, arrivalPoint: selected.uuid }));
    }
  };
  ...
  const handleAdditionalServiceChange = (e: ChangeEvent<HTMLInputElement>, serviceUuid: string) => {
    if (e.target.checked) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      >
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setFormData((prev) => ({ ...prev, departureTime: newDate }));
            }}
          />
        </label>
        <label>
        ...
        <label>
          ...
          <select
            name="arrivalPoint"
            onChange={handleArrivalPointChange}
