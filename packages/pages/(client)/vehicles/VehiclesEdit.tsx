...
const VehiclesEdit: React.FC<VehiclesEditProps> = ({ data }) => {
  const [formData, setFormData] = useState<EditVehicleData>({
    ...
    serviceLevelIds: data.service_levels.map((level) => level.uuid),
  });
  ...
  const handleInputChange = (event: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (id === 'year') {
      setYearInput(value);
    } else if (id === 'serviceLevelIds') {
      setFormData((prevData) => ({
        ...prevData,
        serviceLevelIds: [value],
      }));
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          <label htmlFor="serviceLevelIds">Service Level:</label>
          <select
            id="serviceLevelIds"
            value={formData.serviceLevelIds?.[0] || ''}
            onChange={handleInputChange}
            required
          >
