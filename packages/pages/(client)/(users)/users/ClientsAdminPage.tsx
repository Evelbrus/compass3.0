...
import { usersColumns } from '@pages/(client)/(users)/users/userColums';
import { renderActions } from '@shared/components/ui/table/ui/TableRenders';
import { rolesOverview } from '@pages/(client)/(users)/users/rolesOverview';
...
const ClientsAdminPage = (): JSX.Element => {
  ...
  const [roleFilter, setRoleFilter] = useState<UserRole | 'all' | null>('all');
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const roleCountsData: Record<string, number> = { '': data.totalAllRoles };
  ...
  useEffect(() => {
    fetchUsers();
  }, [page, perPage, roleFilter, sortBy, sortOrder, updateFlag]);
  ...
  return (
    ...
    >
      ...
      <StatusOverview
        ...
        onSelectStatus={(status) => setRoleFilter(status as UserRole)}
