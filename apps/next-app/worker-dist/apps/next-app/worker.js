...
async function processPreOrderNotificationJob(job) {
    ...
    await prisma.$transaction(async (prisma) => {
        ...
        if (delay > 0) {
            ...
            console.log(`📅 Задача checkOverdue добавлена для заказа ${order.uuid}, на 1 минуту до departureTime`);
        }
        else {
            console.warn(`⚠️ Для заказа ${order.uuid} время для выполнения задачи checkOverdue уже прошло`);
        }
    });
    ...
    await prisma.$transaction(async (prisma) => {
        if (order.status !== OrderStatus.OVERDUE && new Date(order.departureTime) < new Date()) {
            ...
            if (notification) {
                ...
                console.log(`⚠️ Заказ ${order.uuid} обновлен до OVERDUE, уведомление обновлено, isRead установлено в false.`);
            }
        }
            ...
            if (notification) {
                ...
                console.log(`⏳ Заказ ${order.uuid}: уведомление обновлено за 1 минуту до departureTime: ${notification.message}`);
            }
        }
    });
