...
import { Action, OrderStatus, DriverAcceptanceStatus, DriverStatus, UserRole, } from '@prisma/client';
...
async function processNotificationJob(order) {
    ...
    await prisma.$transaction(async (prismaTx) => {
        ...
        if (order.assignedDriverId) {
            ...
            console.log('📡 Уведомление inProgress отправлено водителю через сокет:', driverNotificationData);
        }
        else {
            console.warn(`Заказ ${order.uuid} не имеет назначенного водителя — уведомление водителю не отправлено.`);
        }
        //Уведомление клиенту
        ...
        console.log('📡 Уведомление inProgress отправлено клиенту через сокет:', clientNotificationData);
